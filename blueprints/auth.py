...
# The second, __name__, helps locate the blueprint's resources.
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if user and check_password_hash(user.password_hash, password):
                # This logic ensures roles from the old DB are mapped correctly
                ...
                return redirect(url_for('home')) # Note: This will redirect to the 'home' route in app.py
            else:
...
def logout():
    ...
    return redirect(url_for('auth.login')) # Use 'auth.login' to reference the blueprint route
...
def register():
    if request.method == 'POST':
        ...
        try:
            ...
            # --- Your existing registration validation and logic ---
...
def change_password():
    # This assumes a login_required decorator will be applied when registering the blueprint
    # For simplicity here, we proceed, but in a real app you'd protect this route.
    db = SessionLocal()
    if request.method == 'POST':
        try:
