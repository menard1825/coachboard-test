...
def add_focus(player_name):
    ...
    socketio.emit('dev_focus_add', {'focus': model_to_dict(new_focus), 'player_name': player.name})
    return redirect(url_for('home', _anchor='player_development'))
...
def update_focus(focus_id):
    ...
    socketio.emit('dev_focus_update', {'focus': model_to_dict(focus_item), 'player_name': focus_item.player.name})
    return redirect(url_for('home', _anchor='player_development'))
...
def delete_focus(focus_id):
    ...
    if focus_item and focus_item.team_id == session['team_id']:
        player_name = focus_item.player.name
        db.session.delete(focus_item)
        db.session.commit()
        flash('Focus deleted successfully.', 'success')
        socketio.emit('dev_focus_delete', {'focus_id': focus_id, 'player_name': player_name})
    else:
...
def update_lesson_info(player_id):
    ...
    socketio.emit('roster_update', {'player': model_to_dict(player)})
    return redirect(url_for('home', _anchor='player_development'))
