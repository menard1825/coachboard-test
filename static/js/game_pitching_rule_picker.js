(() => {
  ...
  const RULE_INFO = {
    ...
    'Bullpen Tournaments': {
      ...
      short: 'Event preset',
      description: 'Uses Bullpen’s published event preset. Verify the specific tournament rules when needed.',
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      #game-pitching-rules-v2 .gpr-explain,#game-pitching-rules-v2 .gpr-arm-box{margin-top:9px;padding:9px 10px;border:1px solid #e4e7ec;border-radius:10px;background:#fff}
      #game-pitching-rules-v2 .gpr-explain{background:#f8fafc}
      ...
      @media(max-width:575.98px){
        ...
        #game-pitching-rules-v2 .gpr-badge{display:none}
  ...
  function armPlaceholder(data) {
    const ruleSet = data?.arm_care_rule_set || null;
    return {
      enabled: Boolean(ruleSet),
      rule_set: ruleSet,
      players: {},
      loading: Boolean(ruleSet),
    };
  }

  function armCareConcernMarkup(armData) {
    if (armData?.loading) return '<div class="gpr-arm-note text-muted">Loading arm-care guidance…</div>';
    if (!armData?.enabled) return '<div class="gpr-arm-note text-muted">Arm-care guidance is off for this team.</div>';
    const concerns = Object.entries(armData.players || {}).filter(([, item]) => item.status && item.status !== 'Available');
    if (!concerns.length) return '<div class="gpr-arm-note"><span class="badge text-bg-success me-1">On Track</span>No Pitch Smart rest flags for tracked pitchers.</div>';
    ...
    const more = concerns.length > 3
      ? `<div class="gpr-arm-player text-muted">+${concerns.length - 3} more pitcher${concerns.length - 3 === 1 ? '' : 's'} need attention</div>`
      : '';
    return items + more;
  }

  function armCardMarkup(item, armData) {
    if (armData?.loading) return `<span class="gpa-label">Arm care · ${esc(armData.rule_set || 'Pitch Smart')}</span><strong class="text-muted">Loading…</strong>`;
  ...
  function buildAvailabilityCards(data, armData) {
    ...
    card.querySelector('.gpa-shell')?.remove();
    const cards = rows.map(row => {
      ...
      const decision = status.available
        ...
        : (!competitionSelected
          ...
          : (status.next && status.next !== 'Today' ? `Can pitch again: ${status.next}` : status.label));
  ...
  function decorateArmCare(armData) {
    latestArmCare = armData;
    const armBox = document.querySelector('#game-pitching-rules-v2 .gpr-arm-box');
    if (armBox) {
      armBox.innerHTML = `<div class="gpr-arm-box-title">Separate Arm-Care Check · ${esc(armData.rule_set || latestRules?.arm_care_rule_set || 'Off')}</div>${armCareConcernMarkup(armData)}`;
    }
    document.querySelectorAll('#pitcher-availability-card .gpa-card[data-player-name]').forEach(card => {
      const slot = card.querySelector('.gpa-arm');
      if (!slot) return;
      slot.innerHTML = armCardMarkup((armData.players || {})[card.dataset.playerName], armData);
    });
  }

  function renderRulePicker(data, armData) {
    latestRules = data;
    ...
    const defaultOption = teamDefault ? `Use Team Default — ${optionLabel(teamDefault)}` : 'No game rule selected';

    card.classList.toggle('needs-rule', !effective);
    card.innerHTML = `
      <div class="gpr-summary">
        <div class="gpr-summary-copy">
          <div class="gpr-line"><span class="gpr-label">Competition</span><span class="gpr-rule">${esc(effectiveInfo.label)}</span><span class="gpr-badge ${sourceClass}">${esc(source)}</span></div>
          <div class="gpr-line"><span class="gpr-label">Arm Care</span><span class="gpr-arm">${armCare ? esc(armCare) : 'Off'}</span></div>
        </div>
      ...
      <div class="gpr-editor" id="game-pitch-rule-editor-v2" hidden>
        <div class="gpr-head">
          <div><div class="gpr-title">Which competition pitching rules apply to this game?</div><div class="gpr-help">Choose the tournament or league rules that determine official eligibility. Arm-care guidance stays separate and does not change when you choose a different event rule.</div></div>
        ...
        <div class="gpr-explain"><strong>${esc(effectiveInfo.label)}</strong><span>${esc(effectiveInfo.description)}</span><span>Competition reference: ${esc(effectiveInfo.reference)}</span></div>
        <div class="gpr-arm-box"><div class="gpr-arm-box-title">Separate Arm-Care Check · ${esc(armCare || 'Off')}</div>${armCareConcernMarkup(armData)}</div>
      </div>`;
    ...
    buildAvailabilityCards(data, armData);
  }
  ...
  async function loadArmCare() {
    try {
      ...
      if (!response.ok || data.status === 'error') throw new Error(data.message || 'Unable to load arm-care guidance.');
      decorateArmCare(data);
    } catch (error) {
      console.warn('Arm-care guidance unavailable; competition eligibility remains usable.', error);
      const fallback = {
        enabled: Boolean(latestRules?.arm_care_rule_set),
        rule_set: latestRules?.arm_care_rule_set || null,
        players: {},
        unavailable: true,
      };
      decorateArmCare(fallback);
    }
  }

  async function load() {
    try {
      const rulesResponse = await fetch(`/api/game-day/${gameId}/pitching-rules`, {cache:'no-store'});
      ...
      const placeholder = armPlaceholder(data);
      renderRulePicker(data, placeholder);
      loadArmCare();
    } catch (error) {
      console.error('Unable to load competition pitching rules:', error);
      // Preserve the server-rendered availability table if the official rules API
      // fails. Do not hide or replace usable eligibility information with blanks.
    }
  }
