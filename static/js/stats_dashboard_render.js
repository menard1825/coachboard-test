(() => {
  ...
  function flags(row){
    return row.flags?.length ? row.flags.map(f=>`<span class="sv2-flag"><i class="bi bi-info-circle"></i>${esc(usageNoteLabel(f))}</span>`).join('') : '<span class="sv2-good"><i class="bi bi-check-circle me-1"></i>No usage notes</span>';
  }
  ...
  function playerUsage(data){
    ...
    const desktop=`<div class="table-responsive sv2-desktop"><table class="table table-hover sv2-table"><thead><tr><th>Player</th><th>Avail. Games</th><th>Def. Games</th><th>Field Inn.</th><th>Bench Inn.</th><th>Bench %</th><th>Positions</th><th>Usage Notes</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${playerButton(r)}</td><td>${r.available_games}</td><td>${r.defensive_games}</td><td><strong>${r.field_innings}</strong></td><td>${r.bench_innings}</td><td>${val(r.bench_pct)}${r.bench_pct==null?'':'%'}</td><td>${positions(r)}</td><td>${flags(r)}</td></tr>`).join('')}</tbody></table></div>`;
    ...
    return section('Player Usage','Recorded defensive inning appearances and bench usage. Usage Notes highlight patterns for coaches to review — they are not player grades. Mid-inning substitutions count each player who appeared in that inning once.',desktop+mobile);
  }
