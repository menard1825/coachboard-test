...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function playerTemplate(p) {
        ...
        const anchor = window.location.hash || '#roster';
        const deleteButtonHtml = `<button type="button" class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#confirmDeleteModal" data-delete-url="/delete_player/${p.id}?anchor=${anchor.substring(1)}" data-delete-name="${pNameSafe}">Delete</button>`;
    ...
    function renderLineups() {
        ...
        container.innerHTML = lineups.length === 0 
            ...
            : lineups.map((l) => {
                ...
                const anchor = window.location.hash || '#lineups';
                const deleteButtonHtml = `<button type="button" class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#confirmDeleteModal" data-delete-url="/delete_lineup/${l.id}?anchor=${anchor.substring(1)}" data-delete-name="${escapeHTML(l.title)}">Delete</button>`;
    ...
    function renderRotations() {
        ...
        container.innerHTML = rotations.length === 0 ? `<div class="text-center p-4 border rounded"><p class="mb-0">No unassigned rotations saved.</p><p class="small text-muted">Create rotations from the 'Manage' screen of any game.</p></div>` : rotations.map((r) => {
            ...
            const anchor = window.location.hash || '#rotations';
            const deleteButtonHtml = `<button type="button" class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#confirmDeleteModal" data-delete-url="/delete_rotation/${r.id}?anchor=${anchor.substring(1)}" data-delete-name="${escapeHTML(r.title)}">Delete</button>`;
    ...
    function renderSigns() {
        ...
        container.innerHTML = signs.length > 0 ? signs.map((sign) => {
            const anchor = window.location.hash || '#signs';
            const deleteButtonHtml = `<button type="button" class="btn btn-sm btn-danger ms-2" data-bs-toggle="modal" data-bs-target="#confirmDeleteModal" data-delete-url="/delete_sign/${sign.id}?anchor=${anchor.substring(1)}" data-delete-name="the '${escapeHTML(sign.name)}' sign">Delete</button>`;
    ...
    function renderGames() {
        ...
        container.innerHTML = games.map(game => {
            ...
            const anchor = window.location.hash || '#games';
            const deleteButtonHtml = `<button type="button" class="btn btn-outline-danger" data-bs-toggle="modal" data-bs-target="#confirmDeleteModal" data-delete-url="/delete_game/${game.id}?anchor=${anchor.substring(1)}" data-delete-name="the game against ${escapeHTML(game.opponent)}"><i class="bi bi-trash"></i></button>`;
    ...
    function renderPracticePlans() {
        ...
        container.innerHTML = plans.map(plan => {
            ...
            <div class="row mt-4">
                ...
                <div class="col-lg-6">
                    ...
                    <div class="weather-widget" data-location="${escapeHTML(plan.location || '')}" data-date="${plan.date.split('T')[0]}">
        ...
        document.querySelectorAll('.accordion-collapse').forEach(collapseEl => {
            collapseEl.addEventListener('shown.bs.collapse', event => {
                ...
                if (weatherWidget) {
                    fetchPracticeWeather(weatherWidget);
                }
            });
        });
    }
    ...
    function renderCollaborationNotes() {
        ...
        const renderNotesList = (notes, noteType) => {
            ...
            return notes.sort((a, b) => b.timestamp.localeCompare(a.timestamp)).map(note => {
                ...
                const anchor = window.location.hash || '#collaboration';
                const deleteButton = canUserEdit ? `<form action="/delete_note/${noteType}/${note.id}?anchor=${anchor.substring(1)}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this note?');"><input type="hidden" name="csrf_token" value="${csrfToken}"/><button type="submit" class="btn btn-sm btn-link text-danger py-0">Delete</button></form>` : '';
